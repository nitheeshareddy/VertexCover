...
void pathGenerator(std::vector<std::string> tokens, GenGraph g) {
	...
	if (!path_validation.empty()) {
		...
		std::cout << "Error: Provided vertex doesnot exist or path doesnot exist between the given two vertices." << std::endl;
	}
}
	...
	for (unsigned index = 0; index < vertices.size(); ++index) {
		if (index % 2 == 0) {
			if ((std::stoi(vertices[index]) >= number) || (std::stoi(vertices[index + 1]) >= number)) {
				std::cout << "Error: Invalid input! Given Vertex id is larger compared to the size of the graph." << std::endl;
			}
			else {
